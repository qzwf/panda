...
def unpack_can_buffer(dat):
    ...
    while len(dat) >= CANPACKET_HEAD_SIZE:
        ...
        bus = (header[0] >> 1) & 0x7
        address = (header[4] << 24 | header[3] << 16 |
                   header[2] << 8 | header[1]) >> 3
        ...
        assert calculate_checksum(
            dat[:(CANPACKET_HEAD_SIZE+data_len)]) == 0, "CAN packet checksum incorrect"
...
def ensure_version(desc, lib_field, panda_field, fn):
    ...
    def wrapper(self, *args, **kwargs):
        ...
        if lib_version != panda_version:
            raise RuntimeError(
                f"{desc} packet version mismatch: panda's firmware v{panda_version}, library v{lib_version}. Reflash panda.")
        return fn(self, *args, **kwargs)
    return wrapper


ensure_can_packet_version = partial(
    ensure_version, "CAN", "CAN_PACKET_VERSION", "can_version")
ensure_can_health_packet_version = partial(
    ensure_version, "CAN health", "CAN_HEALTH_PACKET_VERSION", "can_health_version")
ensure_health_packet_version = partial(
    ensure_version, "health", "HEALTH_PACKET_VERSION", "health_version")
...
class Panda:
    ...
    F4_DEVICES = [HW_TYPE_WHITE_PANDA, HW_TYPE_GREY_PANDA,
                  HW_TYPE_BLACK_PANDA, HW_TYPE_UNO, HW_TYPE_DOS]
    H7_DEVICES = [HW_TYPE_RED_PANDA, HW_TYPE_RED_PANDA_V2,
                  HW_TYPE_TRES, HW_TYPE_CUATRO]

    INTERNAL_DEVICES = (HW_TYPE_UNO, HW_TYPE_DOS, HW_TYPE_TRES, HW_TYPE_CUATRO)
    HAS_OBD = (HW_TYPE_BLACK_PANDA, HW_TYPE_UNO, HW_TYPE_DOS,
               HW_TYPE_RED_PANDA, HW_TYPE_RED_PANDA_V2, HW_TYPE_TRES, HW_TYPE_CUATRO)
    ...
    def connect(self, claim=True, wait=False):
        ...
        while self._handle is None:
            ...
            self._context, self._handle, serial, self.bootstub, bcd = self.usb_connect(
                self._connect_serial, claim=claim, no_error=wait)
            if self._handle is None:
                self._context, self._handle, serial, self.bootstub, bcd = self.spi_connect(
        ...
        missing_hw_type_endpoint = self.bootstub and ret.startswith(
            b'\xff\x00\xc1\x3e\xde\xad\xd0\x0d')
        ...
        self._assume_f4_mcu = (
            self._bcd_hw_type is None) and missing_hw_type_endpoint
        ...
        try:
            ...
            try:
                ...
                dat = handle.controlRead(
                    Panda.REQUEST_IN, 0xc3, 0, 0, 12, timeout=100)
            ...
            for device in context.getDeviceList(skip_on_error=True):
                if device.getVendorID() in cls.USB_VIDS and device.getProductID() in cls.USB_PIDS:
                    ...
                    if serial is None or this_serial == serial:
                        logger.debug("opening device %s %s",
                                     this_serial, hex(device.getProductID()))
                    ...
                    if device.getVendorID() in cls.USB_VIDS and device.getProductID() in cls.USB_PIDS:
                        try:
                            ...
                            if len(serial) == 24:
                                ...
                                logger.warning(
                                    f"found device with panda descriptors but invalid serial: {serial}", RuntimeWarning)
                        except Exception:
        ...
        try:
            if enter_bootloader:
                self._handle.controlWrite(
                    Panda.REQUEST_IN, 0xd1, 0, 0, b'', timeout=timeout, expect_disconnect=True)
            else:
                if enter_bootstub:
                    self._handle.controlWrite(
                        Panda.REQUEST_IN, 0xd1, 1, 0, b'', timeout=timeout, expect_disconnect=True)
                else:
                    self._handle.controlWrite(
                        Panda.REQUEST_IN, 0xd8, 0, 0, b'', timeout=timeout, expect_disconnect=True)
        except Exception:
            pass
    ...
    def flash_static(handle, code, mcu_type):
        ...
        last_sector = next(
            (i + 1 for i, v in enumerate(apps_sectors_cumsum) if v > len(code)), -1)
        ...
        try:
            handle.controlWrite(Panda.REQUEST_IN, 0xd8, 0,
                                0, b'', expect_disconnect=True)
        except Exception:
            pass
    ...
    def health(self):
        dat = self._handle.controlRead(
            Panda.REQUEST_IN, 0xd2, 0, 0, self.HEALTH_STRUCT.size)
    ...
    def can_health(self, can_number):
        ...
        dat = self._handle.controlRead(Panda.REQUEST_IN, 0xc2, int(
            can_number), 0, self.CAN_HEALTH_STRUCT.size)
    ...
    def get_interrupt_call_rate(self, irqnum):
        dat = self._handle.controlRead(
            Panda.REQUEST_IN, 0xc4, int(irqnum), 0, 4)
    ...
    def set_alternative_experience(self, alternative_experience):
        self._handle.controlWrite(
            Panda.REQUEST_OUT, 0xdf, int(alternative_experience), 0, b'')
    ...
    def set_can_enable(self, bus_num, enable):
        ...
        self._handle.controlWrite(
            Panda.REQUEST_OUT, 0xf4, int(bus_num), int(enable), b'')
    ...
    def set_can_data_speed_kbps(self, bus, speed):
        self._handle.controlWrite(
            Panda.REQUEST_OUT, 0xf9, bus, int(speed * 10), b'')

    def set_canfd_non_iso(self, bus, non_iso):
        self._handle.controlWrite(
            Panda.REQUEST_OUT, 0xfc, bus, int(non_iso), b'')
    ...
    def set_uart_callback(self, uart, install):
        self._handle.controlWrite(
            Panda.REQUEST_OUT, 0xe3, uart, int(install), b'')
    ...
    def can_recv(self):
        ...
        while True:
            try:
                # Max receive batch size + 2 extra reserve frames
        ...
        msgs, self.can_rx_overflow_buffer = unpack_can_buffer(
            self.can_rx_overflow_buffer + dat)
        return msgs
    ...
    def serial_read(self, port_number):
        ret = []
        while 1:
            lret = bytes(self._handle.controlRead(
                Panda.REQUEST_IN, 0xe0, port_number, 0, 0x40))
            if len(lret) == 0:
                break
            ret.append(lret)
        return b''.join(ret)
    ...
    def serial_write(self, port_number, ln):
        ...
        for i in range(0, len(ln), 0x20):
            ret += self._handle.bulkWrite(2, struct.pack("B",
                                          port_number) + ln[i:i + 0x20])
        return ret
    ...
    def set_ir_power(self, percentage):
        self._handle.controlWrite(
            Panda.REQUEST_OUT, 0xb0, int(percentage), 0, b'')

    # ******************* Fan ******************
    def set_fan_power(self, percentage):
        self._handle.controlWrite(
            Panda.REQUEST_OUT, 0xb1, int(percentage), 0, b'')
    ...
    def force_relay_drive(self, intercept_relay_drive, ignition_relay_drive):
        self._handle.controlWrite(Panda.REQUEST_OUT, 0xc5, (int(
            intercept_relay_drive) | int(ignition_relay_drive) << 1), 0, b'')
